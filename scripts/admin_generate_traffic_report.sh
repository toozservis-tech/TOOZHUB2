#!/usr/bin/env bash
# GoAccess HTML z nginx access logu (Správa vozidel — admin).
set -euo pipefail
...
main() {
  ...
  if [[ -z "${src}" ]] || [[ ! -r "${src}" ]]; then
    log_line "ERROR: access log unreadable. Set NGINX_ACCESS_LOG nebo skupinu adm."
    write_meta 0 "${src}" "Access log není dostupný. Nastavte NGINX_ACCESS_LOG nebo oprávnění ke čtení /var/log/nginx/*.log."
    exit 0
  fi

  if ! command -v "${GOACCESS_BIN}" >/dev/null 2>&1; then
    log_line "ERROR: ${GOACCESS_BIN} missing"
  ...
  local ga_opts=(
    ...
    --anonymize-ip
    --no-query-string
    --html-report-title="Správa vozidel — návštěvnost (nginx)"
    --output "${tmp_html}"
    --no-parsing-spinner
    --no-progress
  )
  ...
  if [[ "${ga_ec}" -ne 0 ]]; then
    log_line "ERROR: goaccess exit ${ga_ec} for ${src}"
    rm -f "${tmp_html}"
    write_meta 0 "${src}" "GoAccess selhal (exit ${ga_ec}). Viz ${LOG_DIAG}."
    exit 0
  fi

  if ! python3 "${APP_ROOT}/scripts/goaccess_apply_cs.py" "${tmp_html}" >>"${LOG_DIAG}" 2>&1; then
    log_line "WARN: goaccess_apply_cs.py selhal — HTML zůstane anglicky"
  fi

  mv -f "${tmp_html}" "${REPORT_HTML}"
  chmod 0640 "${REPORT_HTML}" 2>/dev/null || true
  log_line "OK: ${REPORT_HTML} <= ${src}"
  write_meta 1 "${src}" ""
  exit 0
}
