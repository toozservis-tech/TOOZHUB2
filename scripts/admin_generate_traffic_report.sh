...
# Nevystavuje report na veřejný webroot; výstup do runtime adresáře workspace.
set -euo pipefail
...
resolve_access_log() {
  ...
  local dump picked line path
  if command -v nginx >/dev/null 2>&1; then
    dump="$(nginx -T 2>/dev/null || true)"
    # Konkrétní produkční host; ignorujeme staging.* variantu.
...
main() {
  ...
  if [[ "${ga_ec}" -ne 0 ]]; then
    ...
    rm -f "${tmp_html}"
    write_meta 0 "${src}" "GoAccess selhal (exit ${ga_ec}). Zkontrolujte GOACCESS_LOG_FORMAT nebo formát nginx access logu — viz ${LOG_DIAG}."
    exit 0
  fi
