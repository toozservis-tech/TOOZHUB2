...
def get_current_instance(
    ...
    try:
        ...
        if not tenant_id or not instance_id:
            raise HTTPException(status_code=401, detail="Token neobsahuje tenant_id nebo instance_id")
...
def register_instance(
    ...
    if not tenant:
        ...
        tenant = Tenant(
            name=f"Tenant {payload.license_key}",
    ...
    # Vytvořit instanci
    device_id = payload.device_info.hostname or f"device_{datetime.utcnow().timestamp()}"
    instance = Instance(
        tenant_id=tenant.id,
        device_id=device_id,
