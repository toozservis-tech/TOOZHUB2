...
Technická analytika — oddělená od vozidel a účtů; nevrací raw access.log.
"""
...
def traffic_report_html(
    ...
    if not p.is_file() or p.stat().st_size == 0:
        detail_html = (
            ...
            "<p>Spusťte na serveru generování: <code>scripts/admin_generate_traffic_report.sh</code>, "
            "nebo použijte tlačítko „Obnovit přehled“, případně systemd timer "
            "<code>sprava-vozidel-goaccess-report.timer</code>.</p>"
            "</body></html>"
        )
        _audit_traffic(
            db,
