"""Admin traffic report — RBAC a bezpečné chování."""
...
def traffic_rb_client(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(traffic_module, "GOACCESS_REPORT_SCRIPT_PATH", stub_script)
...
def test_service_forbidden(traffic_rb_client):
    c = traffic_rb_client.as_service()
    assert c.get("/admin-api/traffic/report").status_code == 403


def test_admin_get_html(traffic_rb_client):
    ...
    r = traffic_rb_client.as_admin().get("/admin-api/traffic/report")
    assert r.status_code == 200
    assert "text/html" in r.headers.get("content-type", "")
    assert "/var/log/nginx" not in r.text


def test_regenerate_argv(traffic_rb_client, monkeypatch):
    calls = []
    ...
    r = traffic_rb_client.as_admin().post("/admin-api/traffic/report/regenerate?x=1")
    assert r.status_code == 200
    assert calls[0]["shell"] is False
    assert calls[0]["argv"][0] == "/bin/bash"
    assert calls[0]["argv"][1].endswith("admin_generate_traffic_report.sh")
